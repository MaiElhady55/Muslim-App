...
class CustomProvider extends ChangeNotifier {
  ...
  List<HadithDetails> allHadithList = [];
  void loadHadithFile() async {
    List<HadithDetails> hadithList = [];
    String content =
        await rootBundle.loadString('assets/hadith_file/hadith_ar.txt');
    ...
    for (int i = 0; i < allHadithContent.length; i++) {
      ...
      HadithDetails hadith = HadithDetails(title: title, content: content);
...
/*List<AsmaaAllahElHosna> asmaaList = [];
void getAsmaaAllahData() {
  String data = asmaaAllahElHosna;
  List<String> splitData = asmaaAllahElHosna.split('{');
  for (var i=0; i<splitData.length;i++){
    asmaaList.add(
      AsmaaAllahElHosna.fromJson(splitData[i]);
    );
  }
}*/